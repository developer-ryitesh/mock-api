...
export default function AdminLayout() {
   ...
   const { getSession, logout } = useAppSelector((state) => state.user);
               ...
               <div style={{ height: "calc(100vh - 59px)" }}>
                  <div className="h-[58px] bg-white px-5 flex items-center border-gray-300 border-b border-r">
                     <div>
                        ...
                        {props.isToggle && <span className="text-[10px] font-semibold text-[#716F6F] block uppercase">{getSession.data?.role}</span>}
                     </div>
                  </div>
                  <div className="p-2">
                     <PushNotification //

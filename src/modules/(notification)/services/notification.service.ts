...
export default class NotificationService implements IService {
   ...
   subscribe = {
      api: createAsyncThunk("subscribe", async (deviceToken: string, thunkAPI) => {
         try {
            const { data } = await this._repo.subscribeByDeviceToken(deviceToken);
            return data;
         } catch (error) {
   ...
   getAllNotifications = {
      api: createAsyncThunk("!getAllNotifications", async (_, thunkAPI) => {
         try {
            const { data } = await this._repo.getAllNotifications();
            return data;
         } catch (error) {
            return thunkAPI.rejectWithValue(error);
         }
   ...
   markAsRead = {
      api: createAsyncThunk("markAsRead", async (ids: string[], thunkAPI) => {
         try {
            const { data } = await this._repo.markAsRead(ids);
            return data;
         } catch (error) {
            return thunkAPI.rejectWithValue(error);
         }
